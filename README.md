...
> A single-file HTML dashboard for chat analytics — UI preview only.
> 单文件聊天数据分析 dashboard —— 仅 UI 预览。
...
一个**单文件 HTML**，渲染一份用于分析聊天消息统计的 dashboard UI。
此处作为**设计 / 代码参考**公开。
...
- **不是可独立运行的应用。** 配套后端（生成 `messages.jsonl`、提供
  `/api/*` 接口）刻意未包含。直接用 `file://` 或静态服务器打开
